...
Format is auto-detected from the file's `format` field (one format per file;
mixed-format inputs are rejected at load):
...
salt/datasets/README.md). Each turn runs a one-shot selector chosen by
`--selector`: `coverage` (CELF coverage_select, the default; paired here with
options-in-query retrieval and query_mass 2.0) or `legacy` (trie_select, the
original study's configuration). The active selector and its knobs are stamped
into a `kind:"meta"` header row, every index/turn record, and the summary TSV.
Cross-turn coverage memory is a separate concern (see salt.engine.session_trie).
...
# Legacy-selector per-turn knobs. NB: neighbor_window=1 differs from the salt
# CLI default (0); the remaining knobs match the compress.py query-mode defaults.
TURN_SELECT_KW = dict(
...
def run_turn(index, query_text, embed_tok, embed_model, device, compress_pct,
             selector="legacy", lam=0.5, query_mass=1.0):
    """One question turn: embed the query, then select a query-relevant
    context from the already-indexed article (legacy trie_select or the
    CELF coverage_select shipped SALT defaults to)."""
    timings, mem = {}, {}
    _reset_peak(device)
    t_total = time.perf_counter()
    ...
    if clean_words <= word_budget:
        ...
        selected, sel_stats = coverage_select(
            ...
            query_proper_nouns=q_pns, lam=lam, query_mass_ratio=query_mass)
...
def build_parser():
    ...
    ap.add_argument("--selector", choices=["legacy", "coverage"], default="coverage",
                    help="coverage: the CELF selector shipped SALT defaults to "
                         "(default). legacy: the multi-phase trie_select "
                         "heuristic the original study ran.")
    ap.add_argument("--lam", type=float, default=0.5,
                    help="[coverage] CELF discount in (0,1); shipped default 0.5.")
    ap.add_argument("--query-mass", type=float, default=2.0,
                    help="[coverage] query mass as a ratio of document trie mass "
                         "(default 2.0; the salt CLI ships 1.0).")
    ap.add_argument("--no-query-with-options", action="store_false",
                    dest="query_with_options",
                    help="MCQ: drop the four option texts from the retrieval "
                         "query. By default they are appended so selection can "
                         "find option-discriminating evidence; the LLM prompt "
                         "is unchanged either way.")
    ap.set_defaults(query_with_options=True)
...
def main():
    ...
    fmts = {a.get("format", "mcq") for a in articles}
    if len(fmts) > 1:
        raise SystemExit(f"--data mixes formats {sorted(fmts)}; "
                         f"one format per file (build separate subsets)")
    ...
    selector = "none" if args.no_compression else args.selector
    print(f"Loaded {len(articles)} articles | format={fmt} | dataset={src}")
    print(f"Mode: {mode} | selector={selector}")
    ...
    if selector == "coverage":
        selector_kw = {"lam": args.lam, "query_mass_ratio": args.query_mass}
    elif selector == "legacy":
        selector_kw = dict(TURN_SELECT_KW)
    fout.write(json.dumps({
        "kind": "meta", "selector": selector, "selector_kw": selector_kw,
        "format": fmt, "source_dataset": src, "git_commit": git_commit,
        "args": vars(args)}) + "\n")
    rows = []
    ...
    for ai, art in enumerate(articles):
        ...
        for ti, q in enumerate(qs):
            qtext = q["question"]
            rtext = qtext                       # retrieval query; prompt keeps qtext
            if args.query_with_options and q.get("options"):
                rtext = qtext + " " + " ".join(q["options"])
            if args.no_compression:
                ...
                turn = run_turn(index, rtext, embed_tok, embed_model,
                                args.device, args.compress_pct,
                                selector=args.selector, lam=args.lam,
                                query_mass=args.query_mass)
...
def write_summary_tsv(output_path, rows):
    ...
    cols = ["article_id", "turn_idx", "mode", "selector", "format", "index_total_s",
