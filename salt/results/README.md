...
Both build on the tracked engine (`salt.engine.compressor` plus a selector —
legacy `trie_select` or CELF `coverage_select`) and write a per-turn/per-sample
JSONL plus a `*_summary.tsv` next to it.
...
`--selector` picks the per-turn selector: `coverage` (the CELF selector, the
default) or `legacy` (`trie_select`, the original study's configuration). The
active selector and its knobs are
stamped into a `kind:"meta"` header row, every JSONL record, and the summary
TSV, so no emitted number is selector-ambiguous. Accuracy is only meaningful
against a paired `--no-compression` run of the same data/LLM/prompt.
