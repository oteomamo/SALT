...
| `--when off\|auto` | `auto` | `auto` reads a day, month, year or span off the question and holds the conversation rows filed outside it out of that turn's selection, `off` searches every row |
