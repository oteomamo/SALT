...
is a schema that allows exactly one reply. It tries the spelling older
servers take first, `guided_json`, then the newer `structured_outputs`,
and trusts a spelling only when the reply comes back held to that
schema, not merely because the server answered. A newer server answers
a request in the old spelling without complaint and ignores the schema,
so an answer alone proves nothing. The spelling that held the reply is
the one every later schema rides. A model whose server will is told to fill
the schema. One whose server will not is shown the object to copy
instead, which is worth more to it than a paragraph about JSON. And
`/roster probe --deep NAME` will tell you which of the two a given
helper is before you rely on it.
