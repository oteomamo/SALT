...
- **3.0.20** `--add` and `/add` read the model's own `config.json`
...
- **2.11.86** A memory ceiling in tokens. `--memory-cap auto`, the
  ...
  fit alone.
- **2.11.87 - 2.11.95** Scoped search. A session reports how each
  attached file and the conversation relate to a question, whole-file
  and best-sentence similarity with keyword and name hits, shown by
  `--branch-stats`, and `--scope auto` searches only the files a
...
- **2.11.97** `--scope auto` by default. A session with several attached
  ...
  before.
- **2.11.109 - 2.11.115** Scripted runs that scale. `--turns-mode
  independent` gives each item of a scripted run a fresh session of its
  own so no item sees another's memory, with each output row naming the
  session it ran in, while the default, `conversation`, is the one
  session the runs always used. Every turn now reports its time and an
  estimate of what is left, `--turns-timeout` abandons a turn whose
  server went quiet and moves on, and `--turns-resume` continues a
  stopped run from its output file, retrying only the items that failed.
  Each output row now also carries the turn's seconds, the prompt tokens
  and the engine's cache counts, a thinking model's reasoning under
  `think` beside the answer it gave, and the item's other fields. An
  item that carries a reference answer is scored, under the field
  `--turns-gold-field` names or a common one: the row gains `gold`,
  `correct`, the rule that decided it and a token F1, and the run ends
