...
- **2.11.86** A memory ceiling in tokens. `--memory-cap auto`, the
  ...
  fit alone.
- **2.11.87 - 2.11.94** Scoped search. A session reports how each
  ...
  words in that scope with a floor for short files, and `/stats` keeps
  a census of the session's routing. Off by default, and a session
  without attachments selects exactly as before.
