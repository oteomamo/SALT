...
- **2.11.5** `--doc-root`. The MCP server can be started with one folder
  it reads documents from, and `salt_ingest_document` refuses a path
  outside it. Off by default, so a server started as before still reads
  any file it can. Worth setting when the client driving the server is a
